...
public class FinvizDataReader implements ItemReader<ScrapedFinancialData> {
    ...
    public ScrapedFinancialData read() {
        if (!initialized) {
            log.info("Finviz 데이터 스크래핑 시작");
            List<ScrapedFinancialData> data = finvizScrapingService.scrapeAll();
            dataIterator = data.iterator();
            initialized = true;
            log.info("Finviz 데이터 {} 건 로드 완료", data.size());
        }
